...
- State the kernel-integrity assumption behind the provider's SIP and Hardened Runtime guarantee. The README, the threat model and the provider hardware requirements now say the protections hold while the macOS kernel has no unpatched vulnerability that bypasses SIP, and tell operators to install every macOS security update. `docs-check` fails when a restatement drops the qualifier.
